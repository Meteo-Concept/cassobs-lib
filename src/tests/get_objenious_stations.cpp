...
int main()
{
	...
	std::vector<std::tuple<CassUuid, std::string, std::map<std::string, std::string>>> v;
	db.getAllObjeniousApiStations(v);
	for (const auto& s: v) {
		char uuid[CASS_UUID_STRING_LENGTH];
		cass_uuid_string(std::get<0>(s), uuid);
		std::cout << "Station " << uuid << "\n"
			  << "\tObjenious id " << std::get<1>(s) << "\n"
			  ;

		const std::map<std::string, std::string>& variables = std::get<2>(s);
		if (!variables.empty()) {
			for (const auto& s : variables) {
				std::cout << "\tvariable " << s.first << " : " << "objenious var " << s.second << "\n";
			}
		} else {

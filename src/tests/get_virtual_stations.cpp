...
int main()
{
	...
	std::vector<VirtualStation> v;
	db.getAllVirtualStations(v);
	...
	for (const auto& s: v) {
		...
		const std::vector<std::pair<CassUuid, std::vector<std::string>>>& sources = s.sources;
		if (!sources.empty()) {
			for (auto&& [s,vars] : sources) {
				cass_uuid_string(s, uuid);
				std::cout << "\t" << uuid << " : " << "[";
				for (auto&& v: vars) {
					std::cout << v << ", ";
				}

...
namespace meteodata {
	...
	void DbConnectionObservations::prepareStatements()
	{
		...
		_pqConnection.prepare(UPSERT_OBSERVATION,
			...
			" SET "
		        "barometer=COALESCE($3, barometer),"
			"dewpoint=COALESCE($4, dewpoint),"
			"extrahum1=COALESCE($5, extrahum1),"
			"extrahum2=COALESCE($6, extrahum2),"
			"extratemp1=COALESCE($7, extratemp1),"
			"extratemp2=COALESCE($8, extratemp2),"
			"extratemp3=COALESCE($9, extratemp3),"
			"heatindex=COALESCE($10, heatindex),"
			"insidehum=COALESCE($11, insidehum),"
			"insidetemp=COALESCE($12, insidetemp),"
			"leaftemp1=COALESCE($13, leaftemp1),"
			"leaftemp2=COALESCE($14, leaftemp2),"
			"leafwetnesses1=COALESCE($15, leafwetnesses1),"
			"leafwetnesses2=COALESCE($16, leafwetnesses2),"
			"outsidehum=COALESCE($17, outsidehum),"
			"outsidetemp=COALESCE($18, outsidetemp),"
			"rainrate=COALESCE($19, rainrate),"
			"rainfall=COALESCE($20, rainfall),"
			"et=COALESCE($21, et),"
			"soilmoistures1=COALESCE($22, soilmoistures1),"
			"soilmoistures2=COALESCE($23, soilmoistures2),"
			"soilmoistures3=COALESCE($24, soilmoistures3),"
			"soilmoistures4=COALESCE($25, soilmoistures4),"
			"soiltemp1=COALESCE($26, soiltemp1),"
			"soiltemp2=COALESCE($27, soiltemp2),"
			"soiltemp3=COALESCE($28, soiltemp3),"
			"soiltemp4=COALESCE($29, soiltemp4),"
			"solarrad=COALESCE($30, solarrad),"
			"thswindex=COALESCE($31, thswindex),"
			"uv=COALESCE($32, uv),"
			"windchill=COALESCE($33, windchill),"
			"winddir=COALESCE($34, winddir),"
			"windgust=COALESCE($35, windgust),"
			"min_windspeed=COALESCE($36, min_windspeed),"
			"windspeed=COALESCE($37, windspeed),"
			"insolation_time=COALESCE($38, insolation_time),"
			"min_outside_temperature=COALESCE($39, min_outside_temperature),"
			"max_outside_temperature=COALESCE($40, max_outside_temperature),"
			"leafwetnesses_timeratio1=COALESCE($41, leafwetnesses_timeratio1),"
			"soilmoistures10cm=COALESCE($42, soilmoistures10cm),"
			"soilmoistures20cm=COALESCE($43, soilmoistures20cm),"
			"soilmoistures30cm=COALESCE($44, soilmoistures30cm),"
			"soilmoistures40cm=COALESCE($45, soilmoistures40cm),"
			"soilmoistures50cm=COALESCE($46, soilmoistures50cm),"
			"soilmoistures60cm=COALESCE($47, soilmoistures60cm),"
			"soiltemp10cm=COALESCE($48, soiltemp10cm),"
			"soiltemp20cm=COALESCE($49, soiltemp20cm),"
			"soiltemp30cm=COALESCE($50, soiltemp30cm),"
			"soiltemp40cm=COALESCE($51, soiltemp40cm),"
			"soiltemp50cm=COALESCE($52, soiltemp50cm),"
			"soiltemp60cm=COALESCE($53, soiltemp60cm),"
			"leaf_wetness_percent1=COALESCE($54, leaf_wetness_percent1),"
			"voltage_battery=COALESCE($55, voltage_battery),"
			"voltage_solar_panel=COALESCE($56, voltage_solar_panel),"
			"voltage_backup=COALESCE($57, voltage_backup) "
		);
